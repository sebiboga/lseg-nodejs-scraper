...
| `tests/validate-epam-jobs.js` | **LSEG-specific fast validator (used by CI).** HEAD requests only, hardcoded LSEG CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real LSEG API, ANAF, and SOLR |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 39176747. Use only when you need to wipe LSEG jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape LSEG → transform → upsert SOLR → generate docs/jobs.md