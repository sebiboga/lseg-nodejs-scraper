...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('39176747')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('39176747')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '39176747', company: 'LSEG BUSINESS SERVICES RM S.R.L.', brand: 'LSEG' }
      ]));
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:39176747')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.deleteJobsByCIF('39176747')).rejects.toThrow('SOLR delete error: 500');
    });
  });
