...
- **Only the API parsing logic in `index.js`** (`fetchJobsPage`, `parseApiJobs`) is LSEG-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
# E2E tests (real LSEG API, SOLR conditional)
npm run test:e2e
