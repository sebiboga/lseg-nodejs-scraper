...
When you replace `parseApiJobs()` in `index.js` (LSEG JSON API) with a new parser (e.g. `parseHtmlJobs()` for HTML/cheerio), you **must** update `tests/unit/index.test.js` accordingly:

- Replace the `parseApiJobs` test block with tests for the new function
- Update test data fixtures to match the new source format (e.g. HTML fragments instead of LSEG API JSON)
- Update URL generation tests — the URL construction logic changes per data source
- Also update `tests/unit/company.test.js`: replace `LSEG_ANAF_RECORD` and all hardcoded `39176747` / `LSEG BUSINESS INTERNATIONAL SRL` values with the new company's CIF and legal name
- Also update `tests/unit/solr.test.js`: replace all hardcoded `'39176747'`, `'LSEG BUSINESS INTERNATIONAL SRL'`, and `'LSEG'` in mock data with the new company's CIF, legal name, and brand (tests still pass with stale values since mocks are self-referential, but the hardcoded LSEG references are misleading)
- **Also update `tests/integration/workflow.test.js`** and **`tests/e2e/scraper.test.js`**: replace the `TEST_CIF`/`TEST_CIF` constant (`39176747`) with the new CIF, update all hardcoded company name and brand assertions, and replace LSEG API URLs/parsing with the new data source's URL and parser function name
